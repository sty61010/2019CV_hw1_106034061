...
def process_one_image(img_path, pattern):
    ...
    print("I1. Inverse Tone Mapping")
    img = tone_mapping(img, I_inv, B_inv, index=tone_index, inv=True)
    cv2.imshow('I1. Inverse Tone Mapping', img)
    print("I2. from RGB to CIE XYZ")
    img = RGB2XYZ(img)
    cv2.imshow('I2. from RGB to CIE XYZ', img)

    print("I3. Color Correction")
    img = color_correction(img, ccm)
    cv2.imshow('I3. Color Correction', img)

    print("I4. Mosaic")
    img = mosaic(img, pattern=pattern)
    cv2.imshow('I4. Mosaic', img)

    print("I5: Inverse AWB")
    wb_mask = generate_wb_mask(img, pattern, fr, fb)
    img = img * wb_mask
    cv2.imshow('I5: Inverse AWB', img)
